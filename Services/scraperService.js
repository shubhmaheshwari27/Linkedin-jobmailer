...
const scrape = async (io) => {
  try {
    ...
    //   console.log(`Completed scroll ${i + 1}`);
    ...
    for (let i = 0; i < process.env.SCROLL_LIMIT; i++) {
      ...
      const debugInfo = await page.evaluate(() => {
        ...
        return {
          ...
          totalPosts: document.querySelectorAll('div[role="listitem"]').length
        };
    ...
    const posts = await page.evaluate(() => {
      const totalPosts = document.querySelectorAll('div[role="listitem"]').length;
